...
#'
#' Returns zone IDs belonging to specified subbasins by reading the NB_ and NZ_
#' columns from the parameter file (e.g., para.txt). The parameter filename is
#' determined from defaults.txt (PARAFILE entry). If \code{defaults_settings}
#' contains a \code{PARAFILE} override, that takes precedence because
#' defaults.txt has not yet been updated at the time this function is called
#' during optimization setup.
#'
#' @param cosero_path COSERO project path
#' @param subbasins Subbasin IDs (character or numeric), or "all" for all subbasins
#' @param defaults_settings Optional list of COSERO settings passed by the
#'   optimizer. If it contains \code{PARAFILE}, that filename takes precedence
#'   over what is currently in defaults.txt (since defaults.txt is only updated
#'   later by \code{run_cosero()}). If NULL, the filename is read from
#'   defaults.txt, falling back to "para.txt".
...
get_zones_for_subbasins <- function(cosero_path, subbasins = "all",
  ...
  if (!file.exists(par_file)) {
    if (!quiet) message("Parameter file not found: ", par_file, " - will modify all zones")
    return(list(zones = NULL, subbasins = subbasins))
  }

  # Read parameter file to get NB_ (subbasin) and NZ_ (zone) columns
  param_data <- read_cosero_parameters(par_file, quiet = TRUE)

  if (!"NB_" %in% colnames(param_data) || !"NZ_" %in% colnames(param_data)) {
    if (!quiet) message("Parameter file missing NB_ or NZ_ columns - will modify all zones")
    return(list(zones = NULL, subbasins = subbasins))
  }

  # Get all available subbasins
  all_subbasins <- unique(param_data$NB_)
  ...
  if (length(missing) > 0) {
    warning("Subbasins not found in parameter file: ", paste(missing, collapse = ", "),
            call. = FALSE)
  }

  # Get zones for requested subbasins
  zones <- param_data$NZ_[param_data$NB_ %in% subbasin_nums]
...
#' zone-subbasin mapping via the parameter file.
#'
...
#'   using the parameter file. Use this for manual control, e.g., to optimize only
...
#' Reads the NB_ (subbasin) and NZ_ (zone) columns from the parameter file
...
#' Supports single/multi-objective optimization, multiple subbasins, and automatic
#' zone-subbasin mapping via the parameter file. Requires the \code{rtop} package.
#'
